...
trait ErrorStrategy {
  ...
  def apply[R, A](z: ZIO[R, DbException, A]): ZIO[R, DbException, A]
  ...
  def timeout(d: Duration): ErrorStrategy = new ErrorStrategy {
    override def apply[R, A](z: ZIO[R, DbException, A]): ZIO[R, DbException, A] =
...
object ErrorStrategy extends ErrorStrategy {
  ...
  override def apply[R, A](z: ZIO[R, DbException, A]): ZIO[R, DbException, A] = z
